          ...
          <div className="flex flex-wrap gap-2 md:gap-3">
            {participants.slice(0, 24).map((p: any) => (
              <div key={p.id} className="flex items-center gap-2 rounded-full border px-2 py-1">
                <div className="relative h-6 w-6 md:h-8 md:w-8 rounded-full overflow-hidden border">
                  {p.image && <Image src={p.image} alt="avatar" fill />}
                </div>
                <div className="text-[11px] md:text-xs font-medium truncate max-w-[6rem] md:max-w-[8rem]">{p.name}</div>
              </div>
            ))}
          </div>
        </div>
